...
- improve：离成交最有帮助的 2–6 条改进，problem 说清楚错过了什么（涉及「没回、没给、没追、没约」的，按上面的要求写「当前记录里没看到」并附时间证据），better 写出具体可以照着说的话。宁可少写，不写建立在没看全的事实上的条目。
- good：今天做得好的 1–2 段，why 说明好在哪里、以后要保持。
- tomorrow：明天最该先联系的 2–4 个客户，why 说原因，opener 给一句可以直接发的开场白。
- customers：本次窗口里聊过的每个客户各一行，status 一句话说清楚这个客户现在处在哪个阶段（了解产品/处理顾虑/比较方案/做决定/办手续，或在等某个外部条件）、下一步是什么，level 取「快成交」「推进中」「卡住了」「一般」之一。
