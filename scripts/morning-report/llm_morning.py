...
SYS = """你是「保心上人」的财经晨报主编，对标**财联社/财秘晨报(FEMORNING)**那种机构级日报，为保险规划师及其高净值客户编每日《财经晨报》。
读者多为关注养老、储蓄、资产传承的中高净值人群。语言专业、克制、信息密度极高、内容饱满。

【格式颗粒，必须严格照做(这是财秘晨报的标志)】
A. 每一条都写成「机构信源/主体 ＋ 完整一段」：
   - label = 这条新闻的来源机构或主体(如 财政部 / 统计局 / 央行 / 发改委 / 央视新闻 / 国家能源集团 / 美光科技 / 英伟达 / 日本央行 …)，从原文提取，2-8字。
   - text = 一段完整摘要，**把原文里每一个关键数字、比例、金额、时间、机构表态全部保留写进去**，写成2-4句、80-160字的扎实段落，绝不压成干巴一句。
...
{{""")
    parts = []
    if want_meta:
        parts.append('  "trend": "40字内今日风向，给规划师当天选题/沟通的抓手，可点1-2条主线，关键词<b>加粗</b>"')
    ...
    {"label":"机构/主体", "text":"今日最重磅头条之一，写成完整一段(2-3句、80-150字)，保留全部关键数字，<b>标红</b>核心数据", "id": 原文id}
  ]''')
    if secs:
        parts.append('''  "sections": [
    {
      ...
      "items": [
        {"label":"机构/主体(2-8字)", "text":"完整一段(2-4句、80-160字)，**保留原文每一个数字与细节**，关键数据用<b>标红</b>", "id": 原文id}
      ]
    }
  ]''')
    if want_tail:
        parts.append('''  "insure": [
    {"ic":"💵","tx":"把今日宏观/利率/汇率新闻翻译成对美元储蓄险/分红险客户的配置参考，80字内，<b>关键短语加粗</b>"},
    {"ic":"📈","tx":"把今日股市/经济新闻翻译成对资产配置/养老储备的视角，80字内"},
    {"ic":"🛡️","tx":"避险/稳健配置视角的一句话，80字内"}
  ]''')
        parts.append('''  "review": {
    "title":"晨报纵览小标题(当天最大主线，20字内)",
    "paras":["3段、每段80-140字的深度综述，把当天最重要的几条新闻串成逻辑，只用给定事实，落到对中高净值客户养老与财富配置的启示，中性不荐品"]
  }''')
    schema = ",\n".join(parts)
    reqs = ["要求（务必做满，颗粒度对齐财秘晨报）：",
