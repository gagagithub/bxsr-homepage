...
# ⚠2026-08-31 改晨报时补的 d 字段(发布时刻)**保留**: 9-09 改回日报后窗口虽然收回"今天全天"
#   (d 基本都是"今天HH:MM"), 但 fetch 的 <60 条兜底会放宽到近 24 小时、长假也会跨天,
#   留着 d 才不会让 AI 把昨天的事写成今天。零成本, 别删。
...
def build_user(themes=None, want_meta=False, want_review=False, tip_topic=None):
    ...
    head = (f"""这批真实财经新闻电报覆盖【今天全天】（JSON 数组，id 为编号，t 为内容，**d 为这条新闻的发布时刻**，已含较完整细节，请把同主题多条合并成一条更完整的）：
⚠写时间措辞**一律以 d 为准**：d 是"今天HH:MM"就写"今天/今天上午/今天下午/今天收盘"，讲美股外盘写"昨夜/隔夜"，d 是"近期"的不要写具体时间。**万一有 d 标着"昨天/前天"的(取数兜底放宽窗口时会出现)，就照实写"昨天"，绝不许写成"今天"**。
{NEWS_JSON}
...
def gen_briefs():
    ...
    user = (
        "下面是【已在日报正文用过的新闻之外】剩下的真实新闻电报(JSON 数组, id 为编号, d 为发布时刻,\n"
        "时间措辞以 d 为准: 今天的写'今天'、万一有昨天的就照实写'昨天'):\n"
