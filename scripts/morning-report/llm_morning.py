...
【读者画像，所有解读都对着这群人说】
40-60岁，住一二线城市(京沪、长三角、珠三角为主)，家底殷实但不是金融从业者。
他们最关心三件事：
① 健康——生病住院，进口药能报销；② 养老——自己的养老，提前安排好；③ 传承——用不掉的钱，稳稳传给下一代。
写解读时永远回答一个问题："这条新闻跟我这个岁数、我这个家底有什么关系？"
...
{{""")
    ...
    if want_tail:
        parts.append('''  "insure": [
    {"ic":"🌅","tt":"养老","tx":"从当天新闻里挑跟养老最相关的1-2条(利率/国债收益率/存款/养老金/人口…)，先一句点是哪条新闻，再用大白话讲清对'40-60岁、正在给自己安排养老钱的人'意味着什么，落到中性的配置思路，100-150字，<b>关键短语加粗</b>"},
    {"ic":"🌳","tt":"传承","tx":"从当天新闻里挑跟财富传承/家底保值最相关的(汇率/黄金/资产价格/税费政策/楼市…)，讲清对'手里有几套房、有闲钱想稳稳传下去的家庭'意味着什么，100-150字"},
    {"ic":"💵","tt":"钱袋子","tx":"从当天宏观/利率/汇率/股市新闻出发，讲对家庭资产配置(含美元储蓄险/分红险等中性提法)的参考，100-150字"},
    {"ic":"🏥","tt":"健康","tx":"仅当天确有医疗/医保/药品/健康产业新闻时才给这条，讲清对'看病住院、进口药报销'的影响；没有相关新闻就省略这条"}
  ]''')
        parts.append('''  "review": {
    ...
    if want_tail:
        reqs.append("- insure(今日解读)按上面四个视角给：养老、传承、钱袋子必给，健康有相关新闻才给；每条都必须先挂到当天某条真实新闻上，不许写成放之四海皆准的空话；review 必须写满 3 段。")
