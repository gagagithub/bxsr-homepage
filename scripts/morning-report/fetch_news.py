# -*- coding: utf-8 -*-
"""财经日报·取真实新闻电报(东方财富全球财经快讯, 游标翻页, 带标题+摘要+时间+原文链接)。
...
# ---- 时间窗: 只保留【今天(北京)】的新闻(取数要按它翻页, 故先算) ----
# ⚠2026-09-09 崔伟把「崔伟说投资」发送时间从早上改回下午, 这条随之由晨报(07:00跑)改回
#   日报(16:00跑): 窗口从"昨天全天+今早"收回**"今天全天"** —— 下午4点发的报讲的就是今天白天,
#   带上昨天既冲淡主次(读者一眼看不出哪条是新的), 又白白多吃一倍 token。
#   (历史: 6-25 晨报"近30小时" → 7-28 日报"今天全天" → 8-31 晨报"昨天+今早" → 9-09 回到"今天全天"。)
...
CUTOFF = BJ_TODAY0.strftime("%Y-%m-%d %H:%M:%S")
...
#   200 条只回得到当天上午 11 点左右, **今天早盘那一段仍会缺** —— 所以 8-31 加的游标翻页照留,
#   按 CUTOFF 翻到今天 0 点为止(日报窗口下通常 2-3 页就够, 上限 8 页不变)。
...
#   ② 仍超 MAX_CHARS 就按小时分桶等比抽稀 —— 分桶是为了**保证上午不被下午挤掉**,
#      直接砍尾会把早盘那一段整段砍没。
NOISE = re.compile(
