# -*- coding: utf-8 -*-
"""财经日报·渲染：data.json(行情) + sections.json(AI整理的真实新闻) → morning-report.html(移动端H5)。
...
ctx = dict(
    ...
    # A股时点标签跟着取数时刻走(fetch_cn.py 写入): 日报 16:00 跑时 A股(15:00)已收盘,
    # 自动写"今日收盘"(周末写"最新收盘"); 缺字段时的回退也按日报口径给
