...
for name in THEME_ORDER:
    ...
    has_tip = name == "健康" and bool(tip.get("body"))
    if not its and not insight and not has_tip:
        continue
    ...
    # 健康小课堂
    if has_tip:
        w(f'<section style="margin:12px 4px 4px;padding:13px 15px;background:#fff;'
          f'border:1.5px dashed {dark};border-radius:8px;">')
        w(f'<p style="margin:0 0 4px;font-size:15px;font-weight:800;color:{dark};">🩺 健康小课堂 · 每天懂一点</p>')
        if tip.get("title"):
            w(f'<p style="margin:0 0 8px;font-size:20px;font-weight:800;color:{INK};">{strip_tags(tip["title"])}</p>')
